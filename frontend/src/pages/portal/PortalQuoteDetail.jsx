...
// The portal counterpart to pages/PublicQuote.jsx — same read-only layout
// and accept/decline action, but scoped by the client's own auth token
// (routes/clientPortal.js's GET/POST /quotes/:id) rather than a public
// document token, so it's reachable from the portal's own quotes list
// instead of a one-off emailed link.
export default function PortalQuoteDetail() {
  ...
  async function respond(response) {
    ...
    setError('');
    try {
      await api.portal.quotes.respond(id, response, token);
  ...
  async function handleViewPdf() {
    setError('');
    try {
      await api.portal.quotes.openPdf(id, token);
    } catch (err) {
      setError(err.message);
    }
  }
