...
function TemplateCard({ template, token, onChanged }) {
  ...
  return (
    <div className="px-4 py-10 sm:px-6 lg:px-8">
      ...
      <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
        Customize the default subject/message for every client-facing email, and see a record of what's actually
        been sent. Admin-only for now.
      </p>
      ...
      {templates ? (
        <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
          {templates.map((t) => (
            <TemplateCard key={t.type} template={t} token={token} onChanged={loadTemplates} />
          ))}
        </div>
      ) : (
